#!/usr/bin/env bash
# regen_asn1c_lte.sh — regenerate asn1c UMTS RRC (25.331) sources cleanly.
#
# Workflow:
...
Recommended for UMTS/WCDMA RRC:
  $0 rrc-25.331-r19.asn1
EOF
            exit 0 ;;
        *)
            if [[ ! -f "$1" ]]; then
...
OUTPUT_DIR="$PROJECT_ROOT/parsers/umts/asn1c_umts"
...
echo "    cmake -B build -DUSE_ASN1C_UMTS=ON"
