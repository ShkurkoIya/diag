/*
 * Generated by asn1c-0.9.29 (http://lionet.info/asn1c)
 * From ASN.1 module "InformationElements"
 * 	found in "../../Skeletons/ASN_files/UMTS/rrc-25.331-r19.asn1"
 * 	`asn1c -fcompound-names -pdu=all`
 */

#include "UE-HistoryInformation.h"

static int
memb_ue_InactivityPeriod_constraint_1(const asn_TYPE_descriptor_t *td, const void *sptr,
			asn_app_constraint_failed_f *ctfailcb, void *app_key) {
	long value;
	
	if(!sptr) {
		ASN__CTFAIL(app_key, td, sptr,
			"%s: value not given (%s:%d)",
			td->name, __FILE__, __LINE__);
		return -1;
	}
	
	value = *(const long *)sptr;
	
	if((value >= 1 && value <= 120)) {
		/* Constraint check succeeded */
		return 0;
	} else {
		ASN__CTFAIL(app_key, td, sptr,
			"%s: constraint failed (%s:%d)",
			td->name, __FILE__, __LINE__);
		return -1;
	}
}

static asn_oer_constraints_t asn_OER_memb_ue_InactivityPeriod_constr_2 CC_NOTUSED = {
	{ 1, 1 }	/* (1..120) */,
	-1};
static asn_per_constraints_t asn_PER_memb_ue_InactivityPeriod_constr_2 CC_NOTUSED = {
	{ APC_CONSTRAINED,	 7,  7,  1,  120 }	/* (1..120) */,
	{ APC_UNCONSTRAINED,	-1, -1,  0,  0 },
	0, 0	/* No PER value map */
};
asn_TYPE_member_t asn_MBR_UE_HistoryInformation_1[] = {
	{ ATF_NOFLAGS, 0, offsetof(struct UE_HistoryInformation, ue_InactivityPeriod),
		(ASN_TAG_CLASS_CONTEXT | (0 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_NativeInteger,
		0,
		{ &asn_OER_memb_ue_InactivityPeriod_constr_2, &asn_PER_memb_ue_InactivityPeriod_constr_2,  memb_ue_InactivityPeriod_constraint_1 },
		0, 0, /* No default value */
		"ue-InactivityPeriod"
		},
	{ ATF_POINTER, 3, offsetof(struct UE_HistoryInformation, ueMobilityStateIndicator),
		(ASN_TAG_CLASS_CONTEXT | (1 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_High_MobilityDetected,
		0,
		{ 0, 0, 0 },
		0, 0, /* No default value */
		"ueMobilityStateIndicator"
		},
	{ ATF_POINTER, 2, offsetof(struct UE_HistoryInformation, ul_dataVolumeHistory),
		(ASN_TAG_CLASS_CONTEXT | (2 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_DataVolumeHistory,
		0,
		{ 0, 0, 0 },
		0, 0, /* No default value */
		"ul-dataVolumeHistory"
		},
	{ ATF_POINTER, 1, offsetof(struct UE_HistoryInformation, dl_dataVolumeHistory),
		(ASN_TAG_CLASS_CONTEXT | (3 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_DataVolumeHistory,
		0,
		{ 0, 0, 0 },
		0, 0, /* No default value */
		"dl-dataVolumeHistory"
		},
};
static const int asn_MAP_UE_HistoryInformation_oms_1[] = { 1, 2, 3 };
static const ber_tlv_tag_t asn_DEF_UE_HistoryInformation_tags_1[] = {
	(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))
};
static const asn_TYPE_tag2member_t asn_MAP_UE_HistoryInformation_tag2el_1[] = {
    { (ASN_TAG_CLASS_CONTEXT | (0 << 2)), 0, 0, 0 }, /* ue-InactivityPeriod */
    { (ASN_TAG_CLASS_CONTEXT | (1 << 2)), 1, 0, 0 }, /* ueMobilityStateIndicator */
    { (ASN_TAG_CLASS_CONTEXT | (2 << 2)), 2, 0, 0 }, /* ul-dataVolumeHistory */
    { (ASN_TAG_CLASS_CONTEXT | (3 << 2)), 3, 0, 0 } /* dl-dataVolumeHistory */
};
asn_SEQUENCE_specifics_t asn_SPC_UE_HistoryInformation_specs_1 = {
	sizeof(struct UE_HistoryInformation),
	offsetof(struct UE_HistoryInformation, _asn_ctx),
	asn_MAP_UE_HistoryInformation_tag2el_1,
	4,	/* Count of tags in the map */
	asn_MAP_UE_HistoryInformation_oms_1,	/* Optional members */
	3, 0,	/* Root/Additions */
	-1,	/* First extension addition */
};
asn_TYPE_descriptor_t asn_DEF_UE_HistoryInformation = {
	"UE-HistoryInformation",
	"UE-HistoryInformation",
	&asn_OP_SEQUENCE,
	asn_DEF_UE_HistoryInformation_tags_1,
	sizeof(asn_DEF_UE_HistoryInformation_tags_1)
		/sizeof(asn_DEF_UE_HistoryInformation_tags_1[0]), /* 1 */
	asn_DEF_UE_HistoryInformation_tags_1,	/* Same as above */
	sizeof(asn_DEF_UE_HistoryInformation_tags_1)
		/sizeof(asn_DEF_UE_HistoryInformation_tags_1[0]), /* 1 */
	{ 0, 0, SEQUENCE_constraint },
	asn_MBR_UE_HistoryInformation_1,
	4,	/* Elements count */
	&asn_SPC_UE_HistoryInformation_specs_1	/* Additional specs */
};

