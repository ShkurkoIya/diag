...
bool DiagWcdmaLogParser::parse_resel_rank(const uint8_t* p, size_t plen) {
    ...
    size_t per_3g = (ver == 0) ? 10 : (ver == 1) ? 11 : 16;
...
static uint32_t extract_sib3_cid_from_tree(const std::string& tree) {
    ...
    while (p != std::string::npos) {
        ...
        uint8_t b[4]; int n = 0; unsigned v = 0; int nib = 0;
        for (char c : line) {
            if (c == '(') break;            // stop at "(N bits unused)"
            int h = (c >= '0' && c <= '9') ? c - '0'
                  : (c >= 'A' && c <= 'F') ? c - 'A' + 10
                  : (c >= 'a' && c <= 'f') ? c - 'a' + 10 : -1;
            if (h < 0) { if (nib) break; else continue; }
            v = (v << 4) | h; nib++;
            if (nib == 2) { b[n++] = static_cast<uint8_t>(v); v = 0; nib = 0; if (n == 4) break; }
        }
        if (n == 4) {
