...
bool DiagLteRrcParser::parse(const uint8_t* buf, size_t len) {
    ...
    switch (hdr->code) {
        case LOG_LTE_RRC_SERV_CELL_INFO_C: return parse_rrc_serving_cell_info(p, plen);
        case LOG_LTE_RRC_MIB_C:            return parse_rrc_mib(p, plen);
        case LOG_LTE_RRC_OTA_C:            return parse_rrc_ota(p, plen);
        default:
...
bool DiagLteRrcParser::parse_rrc_serving_cell_info(const uint8_t* p, size_t plen) {
    ...
    if (ver == 3 && plen >= 29) {
        ...
        else                            std::snprintf(mnc_str, sizeof(mnc_str), "%02u", info.mnc);
...
void DiagLteRrcParser::stash_serving_cell() {
    ...
    for (auto& c : serv_cells_) {
        if (c.earfcn == id_.earfcn && c.pci == id_.pci) {
            ...
            c.tac = id_.tac; c.mcc = id_.mcc; c.mnc = id_.mnc;
...
bool DiagLteRrcParser::parse_rrc_ota(const uint8_t* p, size_t plen) {
    ...
    if (ver >= 25) {
        ...
        if (journal_enabled()) {
            const char* chan = "RRC"; const char* msg = "RRC"; const char* pdu_name = nullptr;
            switch (item.pdu_num) {
                case 1:  chan = "BCCH-BCH";    msg = "MasterInformationBlock";       pdu_name = "BCCH-BCH-Message";    break;
                case 3:  chan = "BCCH-DL-SCH"; msg = "SIB1/SystemInformation";       pdu_name = "BCCH-DL-SCH-Message"; break;
                case 6:  chan = "MCCH";        msg = "MCCH";                         pdu_name = "MCCH-Message";        break;
                case 7:  chan = "PCCH";        msg = "Paging";                       pdu_name = "PCCH-Message";        break;
                case 8:  chan = "DL-CCCH";     msg = "DL-CCCH";                      pdu_name = "DL-CCCH-Message";     break;
                case 9:  chan = "DL-DCCH";     msg = "DL-DCCH";                      pdu_name = "DL-DCCH-Message";     break;
                case 10: chan = "UL-CCCH";     msg = "UL-CCCH";                      pdu_name = "UL-CCCH-Message";     break;
                case 11: chan = "UL-DCCH";     msg = "UL-DCCH (MeasurementReport?)"; pdu_name = "UL-DCCH-Message";     break;
                default: break;
            }
            char sbuf[96];
            ...
            jr.code = 0xB0C0; jr.rat = "LTE_RRC";
            jr.channel = chan; jr.msg_type = msg; jr.summary = sbuf;
            ...
            if (item.pdu_num == 3) {
                ...
                else if (!dr.ok)                   jr.msg_type = "SystemInformation (decode failed)";
                if (dr.sib1) {
