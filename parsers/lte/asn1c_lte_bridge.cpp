...
namespace lte_asn1 {
    ...
    const char* pdu_type_name(uint8_t pdu_type_canonical) {
        switch (pdu_type_canonical) {
            case 1: return "BCCH-BCH-Message";
            case 2: return "BCCH-DL-SCH-Message";
            case 3: return "MCCH-Message";
            case 4: return "PCCH-Message";
            case 5: return "DL-CCCH-Message";
            case 6: return "DL-DCCH-Message";
            case 7: return "UL-CCCH-Message";
            case 8: return "UL-DCCH-Message";
                    ...
                    if (si.criticalExtensions.present ==
                        ...
                        for (int i = 0; i < sib_list.list.count; ++i) {
                            ...
                            if (item->present >= SystemInformation_r8_IEs__sib_TypeAndInfo__Member_PR_sib2 &&
                                ...
                                int n = static_cast<int>(item->present)
                                      - static_cast<int>(SystemInformation_r8_IEs__sib_TypeAndInfo__Member_PR_sib2) + 2;
                                if (!sibs.empty()) sibs += ",";
        ...
        if (s.plmn_list.size() != 1) {
            ...
            if (s.plmn_list[0].mcc != "250") {
                std::fprintf(stderr, "  MCC=%s, expected 250\n",
                             s.plmn_list[0].mcc.c_str()); ok = false;
            }
            if (s.plmn_list[0].mnc != "01") {
                std::fprintf(stderr, "  MNC=%s, expected 01\n",
                             s.plmn_list[0].mnc.c_str()); ok = false;
            }
        }
        if (s.tac != 7) {
