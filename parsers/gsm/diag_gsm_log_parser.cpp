...
bool DiagGsmLogParser::parse_gsm_l1_surround_cell_ba(const uint8_t* p, size_t plen) {
    ...
    for(int i = 0; i<num_cells; i++) {
        ...
        if (n.arfcn != 0 && n.rxlev != 0) {
            int16_t _c1 = 0, _c2 = 0;
            if (neighbor_c1c2(n.arfcn, n.rxlev, _c1, _c2)) { n.c1 = _c1; n.c2 = _c2; }
        }
...
static void gsm_emit_journal(uint16_t code,const std::string& label,
    ...
    JournalRecord jr; jr.t=static_cast<double>(time(nullptr));
    jr.code=code; jr.rat="GSM"; jr.channel=label; jr.msg_type=label;
    jr.summary=summary; jr.detail=detail;
    jr.raw=journal_hex(p,plen); jr.len=plen; journal_emit(jr);
}
bool DiagGsmLogParser::parse_gsm_rr(const uint8_t* p, size_t plen) {
    ...
    switch (rr.msg_type) {
        case 0x19: si = "SI-1";    break;
        case 0x1A: si = "SI-2";    break;
        case 0x02: si = "SI-2bis"; break;
        case 0x03: si = "SI-2ter"; break;
        case 0x1B: si = "SI-3";    break;
        case 0x1C: si = "SI-4";    break;
        case 0x1D: si = "SI-5";    break;
        case 0x05: si = "SI-5bis"; break;
        case 0x06: si = "SI-5ter"; break;
        case 0x1E: si = "SI-6";    break;
        default:   si = nullptr;   break;
    }
    ...
    if (rr.msg_type == 0x1B || rr.msg_type == 0x1E) {
        ...
        if (l3_len >= 10) {
            ...
            if (cid != 0 && mcc >= 100 && mcc <= 999) {
                ...
                { char _b[112]; std::snprintf(_b,sizeof(_b),"CID=%u LAC=%u MCC=%u MNC=%u",cid,lac,mcc,mnc); si_detail=_b; }
                GsmCell g{};
                g.cid  = cid;
                g.lac  = lac;
                g.mcc  = mcc;
                ...
                if (serving_.cid == cid && serving_.arfcn != 0) {
                    ...
                    if (si3_sel_valid) arfcn_sel_[g.arfcn] = si3_sel;
                    {
                        ...
                        if (nrx != 0) {
                            g.rxlev = nrx;
                            int16_t _c1 = 0, _c2 = 0;
                            if (neighbor_c1c2(g.arfcn, nrx, _c1, _c2)) { g.c1 = _c1; g.c2 = _c2; }
                        }
                    }
                    char _b2[48]; std::snprintf(_b2,sizeof(_b2)," arfcn=%u(surround)",surround_arfcn_hint_); si_detail += _b2;
                }
    ...
    if (rr.msg_type == 0x1C) {
        ...
        if (l3_len >= 10) {
            ...
            recompute_serving_c1c2();
            uint16_t mcc=0,mnc=0,lac=0; unpack_lai(l3 + 3, mcc, mnc, lac);
            char _b[96]; std::snprintf(_b,sizeof(_b),"LAI MCC=%u MNC=%u LAC=%u",mcc,mnc,lac); si_detail=_b;
        }
    }
