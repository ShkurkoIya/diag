    ...
    if (g_custom_libdiag_path) {
        if (try_load(g_custom_libdiag_path)) {
            DIAG_LOGI("libdiag: loaded from custom path %s (variant %s)",
                      loaded_path_.c_str(),
                      variant_ == ApiVariant::VariantA ? "A" : variant_ == ApiVariant::VariantB ? "B"
                                                                                                : "?");
            return true;
        }
    }
    ...
    for (const char **p = kLibdiagPaths; *p; ++p) {
        if (try_load(*p)) {
            DIAG_LOGI("libdiag: loaded from %s (variant %s)",
                      loaded_path_.c_str(),
                      variant_ == ApiVariant::VariantA ? "A (allocate_client)" : variant_ == ApiVariant::VariantB ? "B (register_dci_client)"
                                                                                                                  : "?");
            return true;
        }
    }
...
bool LibdiagLoader::try_load(const char *path) {
    ...
    if (alloc_b && rel_b) {
        ...
        // Нет ни того ни другого — не наша библиотека
        DIAG_LOGE("libdiag: missing client management symbols in %s "
                  "(no allocate_client AND no register_dci_client)",
                  path);
        unload();
        return false;
    }
    ...
    bool ok = lsm_init != nullptr && lsm_deinit != nullptr && register_dci_stream != nullptr && log_stream_config != nullptr && event_stream_config != nullptr && diag_get_dci_support_list_proc != nullptr;
    if (!ok) {
...
void *LibdiagLoader::resolve(const char *sym, bool mandatory) {
    ...
    if (err) {
        if (mandatory) DIAG_LOGE("libdiag: dlsym(%s) failed: %s", sym, err);
            ...
            DIAG_LOGD("libdiag: optional dlsym(%s) not found", sym);
        return nullptr;
    }
