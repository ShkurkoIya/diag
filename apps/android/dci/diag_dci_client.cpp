...
bool DiagDciClient::start() {
    ...
    if (!step_allocate_client()) {
        state_ = State::Error;
        step_lsm_deinit();
        return false;
    }
    if (!step_register_callbacks()) {
        state_ = State::Error;
        step_release_client();
        step_lsm_deinit();
        return false;
    }
    if (!step_enable_log_mask()) {
        state_ = State::Error;
        step_release_client();
        step_lsm_deinit();
        return false;
...
void DiagDciClient::send_syssel_pref(uint32_t mode_pref, const std::string &plmn_str) {
    LibdiagLoader &loader = LibdiagLoader::instance();
    if (!lsm_inited_) {
        DIAG_LOGW("send_syssel_pref: LSM not inited");
        return;
    }
    if (!loader.send_dci_async_req) {
        DIAG_LOGW("send_syssel_pref: send_dci_async_req unavailable");
        return;
    }
