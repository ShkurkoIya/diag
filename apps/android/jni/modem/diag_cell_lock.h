...
struct BtsLteLockItem {
    uint32_t pci;
    uint32_t freq;
};
struct BtsLteLockList {
    uint32_t len;
    BtsLteLockItem cell[30];
};
...
public:
    ...
    int lock_lte_v1(uint16_t earfcn, uint16_t pci) {
        ...
        uint8_t val[4];
        uint16_t be = __builtin_bswap16(earfcn);
        memcpy(val, &be, 2);
        be = __builtin_bswap16(pci);
        memcpy(val + 2, &be, 2);
...
public:
    ...
    int read_ue_usage_setting() {
        ...
        fprintf(stderr, "[NV] ue_usage_setting = %d (%s)\n", v,
                v == 0 ? "data-centric" : v == 1 ? "voice-centric"
                                                 : "unknown");
        return v;
    }
...
private:
    ...
    int client_id_;
    bool debug_ = false;
    bool lte_locked_ = false;
    uint32_t lte_earfcn_ = 0;
    uint16_t lte_pci_ = 0;
    int errno_ = 0;
    int32_t fd_ = -1;
