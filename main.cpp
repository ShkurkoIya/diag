...
static Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        std::string a = argv[i];
        if (a == "-h" || a == "--help") { print_help(argv[0]); exit(0); }
        else if (a == "-v") o.verbose    = true;
        else if (a == "-n") o.nas_mode   = true;
        else if (a == "-l") o.no_screen  = true;
        else if (a == "-t" && i+1 < argc) o.duration_sec = atoi(argv[++i]);
        else if (a == "-s" && i+1 < argc) o.refresh_sec  = atoi(argv[++i]);
        else if (a == "-j" && i+1 < argc) o.json_path    = argv[++i];
        else if (a == "--journal" && i+1 < argc) o.journal_path = argv[++i];
        ...
        else if (a == "-m" && i + 1 < argc) o.lock_mode = argv[++i];
        else if (a == "-L" && i+1 < argc) o.lock_earfcn_pci = argv[++i];
        else if (a == "-lv" && i+1 <argc) o.lock_version = argv[++i];
        else if (a == "--unlock") o.unlock_cell = true;
        else if (a == "--data-centric")    o.force_data_centric = true;
        else if (a == "--voice-centric")   o.restore_voice_centric = true;
        else if (a == "--read-ue-setting") o.read_ue_setting = true;
        else if (a == "--simless-prep")    o.simless_prep = true;
        else if (a == "--read-mode-pref")  o.read_mode_pref = true;
        ...
        else if (a == "--efs-debug") o.efs_debug = true;
        else if (a == "--log-file" && i+1 < argc) o.log_file = argv[++i];
        else if (a == "-r") o.reboot = true;
        else if (a == "--asn1-selftest") o.asn_selftest = true;
        ...
        else if (a == "--unlock-gsm") { o.unlock_gsm_flag = true; }
        else if (a == "--read-gsm-lock") { o.read_gsm_lock = true; }
        else {
...
// ─────────────────────────────────────────────────────────────────────────────
struct CellRecord {
    enum class Type { GSM, WCDMA, LTE, UMTS, NR } type;
    ...
    std::string signal_bar() const {
        ...
        if (type == Type::LTE || type == Type::NR) {
            ...
            else if (v >= -105) bars = 3;
            else if (v >= -115) bars = 2;
            else                bars = 1;
        } else if (type == Type::WCDMA || type == Type::UMTS) {
            if      (v >= -75)  bars = 5;
            else if (v >= -85)  bars = 4;
            else if (v >= -95)  bars = 3;
            else if (v >= -105) bars = 2;
            else                bars = 1;
        } else {
            if      (v >= -70)  bars = 5;
            else if (v >= -80)  bars = 4;
            else if (v >= -90)  bars = 3;
            else if (v >= -100) bars = 2;
            else                bars = 1;
        }
        std::string s;
    ...
    std::string mccmnc_str() const {
        ...
        else          snprintf(buf, sizeof(buf), "%u%u", mcc, mnc);
        return buf;
    }
    ...
    void upsert(CellRecord rec) {
        ...
        if (it == cells.end()) {
            ...
            if (!identity_locked) {
                ...
                if (rec.mcc     >  0)  { e.mcc = rec.mcc; e.mnc = rec.mnc; }
            }
...
// ─────────────────────────────────────────────────────────────────────────────
struct NasEntry {
    time_t ts; bool uplink; uint8_t pd, msg_type;
    std::string name; size_t pdu_len;
};
struct NasLog {
    std::mutex mx;
...
static std::string fmt_time(time_t t) {
    char buf[32]; struct tm ti; localtime_r(&t, &ti);
    strftime(buf, sizeof(buf), "%H:%M:%S", &ti); return buf;
}
...
static void write_json(const std::string& path, const std::vector<CellRecord>& cells,
    ...
    if (!f) { fprintf(stderr, "[WARN] Cannot write JSON: %s\n", strerror(errno)); return; }

    time_t now = time(nullptr);
    char ts[64]; struct tm ti; gmtime_r(&now, &ti);
    ...
    for (size_t i = 0; i < cells.size(); ++i) {
        ...
        const char* rat_str;
        switch (c.type) {
            case CellRecord::Type::GSM:   rat_str = "GSM";   break;
            case CellRecord::Type::WCDMA: rat_str = "WCDMA"; break;
            case CellRecord::Type::LTE:   rat_str = "LTE";   break;
            case CellRecord::Type::UMTS:  rat_str = "UMTS";  break;
            case CellRecord::Type::NR:    rat_str = "NR";    break;
        }
        fprintf(f,
                "    {\n"
                "      \"rat\": \"%s\",\n"
...
static const char* rat_color(CellRecord::Type t) {
    switch (t) {
        case CellRecord::Type::GSM:   return MAGENTA;
        case CellRecord::Type::WCDMA: return YELLOW;
        case CellRecord::Type::LTE:   return GREEN;
        case CellRecord::Type::UMTS:  return CYAN;
        case CellRecord::Type::NR:    return BLUE;
    }
    return WHITE;
}
static const char* rat_name(CellRecord::Type t) {
    switch (t) {
        case CellRecord::Type::GSM:   return "GSM  ";
        case CellRecord::Type::WCDMA: return "WCDMA";
        case CellRecord::Type::LTE:   return "LTE  ";
        case CellRecord::Type::UMTS:  return "UMTS ";
        case CellRecord::Type::NR:    return "NR   ";
    }
    return "?????";
}
static const char* rsrp_color(int v) {
    if (v >= -85) return GREEN; if (v >= -100) return YELLOW; return RED;
}
...
static void render_nas_screen(const std::vector<NasEntry>& nas_log,
    ...
    if (nas_log.empty()) { printf(" %sNo NAS messages yet...%s\n", DIM, RESET); }
    else {
...
static const char* rat_name(Rat r) {
    switch (r) {
        case Rat::GSM: return "GSM"; case Rat::WCDMA: return "WCDMA";
        case Rat::UMTS: return "UMTS"; case Rat::NR: return "NR";
        case Rat::LTE_RRC: return "LTE_RRC"; case Rat::LTE_NAS: return "LTE_NAS";
        case Rat::LTE: return "LTE"; default: return "UNKNOWN";
    }
}
...
static void log_frame(const DiagFrame& f) {
    ...
    switch (f.rat) {
        case Rat::GSM: col=MAGENTA; break; case Rat::WCDMA: col=YELLOW; break;
        case Rat::LTE: case Rat::LTE_NAS: col=GREEN; break;
        case Rat::UMTS: col=CYAN; break; case Rat::NR: col=BLUE; break;
        default: break;
    }
...
int main(int argc, char** argv) {
    ...
    if (argc >= 2 && std::string(argv[1]) == "setmode") {
        ...
        else if (m == "wcdma") mode = 14;   // WCDMA_ONLY
        else if (m == "lte")   mode = 38;   // LTE_ONLY
        else if (m == "auto")  mode = 2;    // AUTOMATIC
        else { fprintf(stderr, "usage: %s setmode <gsm|wcdma|lte|auto> [mccmnc]\n", argv[0]); return 2; }
        DiagDciClient sm_client;
        if (!sm_client.start()) { fprintf(stderr, "[SETMODE] DCI start failed\n"); return 1; }
    ...
    if (!opts.log_file.empty()) {
        ...
        for (char c : opts.log_file) { if (c == '\'') esc += "'\\''"; else esc += c; }
    ...
    CellDb db; NasLog nas_log; AppStats stats;
    stats.start_time = time(nullptr);
    std::string last_status; std::mutex status_mx;
    ...
    if (!lock_only) {
        if (!(opts.rat_gsm && opts.rat_wcdma && opts.rat_lte && opts.rat_umts && opts.rat_nr)) {
            ...
            for (uint16_t c : all) {
                ...
                if (r == Rat::GSM && opts.rat_gsm) { filtered.push_back(c); continue; }
                if (r == Rat::WCDMA && opts.rat_wcdma) { filtered.push_back(c); continue; }
                if ((r == Rat::LTE || r == Rat::LTE_NAS) && opts.rat_lte) { filtered.push_back(c); continue; }
                if (r == Rat::UMTS && opts.rat_umts) { filtered.push_back(c); continue; }
                if (r == Rat::NR && opts.rat_nr) { filtered.push_back(c); continue; }
            }
        ...
        if (!opts.journal_path.empty()) {
            ...
            else journal_set_sink(journal_sink_to_file);
        }
        client.set_raw_frame_callback([&](const DiagFrame& f) {
            stats.frames_total++;
            switch (f.rat) {
                case Rat::GSM:     stats.frames_gsm++;   break;
                case Rat::WCDMA:   stats.frames_wcdma++; break;
                case Rat::LTE:
                case Rat::LTE_NAS: stats.frames_lte++;   break;
                case Rat::UMTS:    stats.frames_umts++;   break;
                case Rat::NR:      stats.frames_nr++;    break;
                default: break;
            }
            if (opts.verbose && opts.no_screen) log_frame(f);
            { std::lock_guard<std::mutex> _hl(g_code_hist_mx); g_code_hist[f.log_code]++; }
            ...
            if (g_journal && f.log_code != 0xB0C0 && f.log_code != 0x412F &&
                ...
                JournalRecord jr;
                jr.t = f.timestamp_unix; jr.code = f.log_code;
                jr.rat = rat_name(f.rat); jr.len = f.payload_len;
    ...
    printf("%s     %s%s\n", DIM,
           opts.duration_sec ? (std::string("stopping in ") +
                                std::to_string(opts.duration_sec) + "s").c_str()
    ...
    auto final_cells = db.snapshot();
    {
        ...
        std::string hs;
        char hb[40];
        for (const auto& kv : hv) { std::snprintf(hb,sizeof(hb),"0x%04X=%llu ",kv.first,(unsigned long long)kv.second); hs += hb; }
        DIAG_LOGI("CODE-HIST received log codes (code=count): %s", hs.c_str());
        if (g_journal) {
            JournalRecord jr; jr.t=(double)time(nullptr); jr.code=0; jr.rat="-";
            jr.channel="CODE-HIST"; jr.msg_type="histogram";
            jr.summary="received log codes (discovery)"; jr.detail=hs; jr.len=0;
            journal_emit(jr);
        }
    }
